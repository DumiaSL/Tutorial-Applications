...
class Dog_1 : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_all_lost_dogs)
        ...
        val dogs= intent.getIntegerArrayListExtra("dog list")
        ...
        checkButton.setOnClickListener {
            if (dogName.text.isNotEmpty()){
                if (checkButton.text=="Check"){
                    checkButton.text="Next One"
                    if (dogName.text.toString().lowercase()=="golden retriever"){
                        submitP.visibility = View.VISIBLE
                        correct_image.visibility = View.VISIBLE
                        correct++

                    }else{
                        submitP.setTextColor(Color.parseColor("#FF0000"))
                        submitP.text="Wrong"
                        dogName.setText("Golden Retriever")
                        submitP.visibility = View.VISIBLE
